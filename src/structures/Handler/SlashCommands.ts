...
export class SlashCommandsHandler {
    ...
    registerFile(file) {
        ...
        if (file.endsWith(".public.js")) pull.execute = (interaction, client) => {interaction.reply({embeds: [new EmbedBuilder().setColor("DarkOrange").setDescription("Cette commande n'est pas disponible pour la version public !\n\n**[Commander EagleBot](https://discord.gg/eaglebot)**").setFooter({text: "Eagle Bot", iconURL: client.user.avatarURL()})], ephemeral: true})}
        if (pull.data.name) {
