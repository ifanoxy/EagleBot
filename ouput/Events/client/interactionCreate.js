...
exports.default = {
            ...
            case interaction.isChatInputCommand():
                {
                    ...
                    if (logChannel)
                        this.log(client, interaction, logChannel);
                    const command = client.handlers.slashCommandsHandler.SlashCommandsList.get(interaction.commandName);
                    if (interaction.inGuild()) {
                        const NotPerm = client.hasNotPermissions(interaction);
                        if (NotPerm) {
    ...
    log(client, interaction, channel) {
        channel.send({
            embeds: [
                new discord_js_1.EmbedBuilder().setColor("#2f3136").setTimestamp()
                    .setTitle(`Logs | Commands Used`)
                    .setDescription(`**Commande:** </${interaction.commandName}:${interaction.id}>\n\n` +
