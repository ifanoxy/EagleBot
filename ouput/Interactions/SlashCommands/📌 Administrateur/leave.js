...
exports.default = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName("activer").setDescription("Permet d'activer les messages de départ")
        .addChannelOption(opt => opt.setName("channel").setDescription("le channel où seront envoyé les messages de départ").setRequired(true).addChannelTypes(discord_js_1.ChannelType.GuildText))
        ...
        .addSubcommand(sub => sub.setName("désactiver").setDescription("Permet désactiver les messages de départ")))
    ...
    execute(interaction, client) {
        ...
        switch (subGroup) {
            case "setup":
                {
                    if (interaction.options.getSubcommand() == "activer") {
                        ...
                        if (embedName)
                            database.leave.message.embed = client.managers.membersManager.getIfExist(interaction.user.id).embeds[embedName];
                        if (message)
                            database.leave.message.content = message;
            ...
            case "test":
                {
                    if (!database || !((_a = database.leave) === null || _a === void 0 ? void 0 : _a.channelId))
                        return interaction.reply({
                            embeds: [
                                new discord_js_1.EmbedBuilder()
                                    .setColor('Red')
                    ...
                    if (!interaction.guild.channels.cache.get((_b = database.leave) === null || _b === void 0 ? void 0 : _b.channelId)) {
